...
take_every = length(x)/num_semp;
A =zeros(num_semp, length(x));
%y = x;
X = fft(transpose(x));
...
X = fft(transpose(x));
random_sampled =randperm(length(X),num_semp);
A = zeros(num_semp,length(X));
for i=1:num_semp
    if(i==1)
         A(i,i) =1;
...
for i = 1:length(lambdas)
    ...
    figure()
     ...
     title('original x')
    %}
    x_hat_pocs = pocs(X, A, lambdas(i), eps);
